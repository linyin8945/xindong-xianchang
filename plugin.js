...
const state={
  roche:null,container:null,activeTab:"show",page:"tab",stack:[],
  user:null,candidateCharacters:[],characters:[],archives:[],currentArchive:null,
  listeners:[]
};
...
.roche-plugin-xindong-xianchang,.roche-plugin-xindong-xianchang *{box-sizing:border-box}
...
@media(min-width:700px){.roche-plugin-xindong-xianchang{max-width:520px;margin:0 auto!important;border-left:1px solid rgba(120,90,96,.08);border-right:1px solid rgba(120,90,96,.08)}}
...
function render(){
 ...
 else h=state.activeTab==="show"?show():state.activeTab==="guests"?guests():state.activeTab==="watch"?watch():archives();
...
async function save(){
 ...
 const i={archiveId:a.archiveId,title:a.title,currentDay:a.currentDay,characterNames:a.characters.map(x=>x.name),characterSnapshots:a.characters.map(x=>({id:x.id,name:x.name,handle:x.handle,avatar:x.avatar})),lastSummary:a.lastSummary||a.lastNarrative?.slice(0,80)||"暂无剧情",modeLabel:modeText(),lastSavedAt:a.lastSavedAt};
...
window.RochePlugin.register({id:PLUGIN_ID,name:"心动现场",version:"1.2.0",apps:[{id:APP_ID,name:"心动现场",icon:"heart",iconImage:"",async mount(c,r){await mount(c,r)},async unmount(c){await unmount(c)}}]});
})();
